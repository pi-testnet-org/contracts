...
import "../domains/ENS.sol";
import "../domains/Registrar.sol";
import "../domains/HashRegistrar.sol";
import "./ERC721.sol";
...
contract BaseRegistrarImplementation is BaseRegistrar, ERC721 {
    ...
    modifier onlyController {
        require(controllers[msg.sender], "no controller");
        _;
    }
    ...
    function register(uint256 id, address owner, uint duration) external live onlyController returns(uint) {
        require(available(id), "no available 2");
        require(now + duration + GRACE_PERIOD > now + GRACE_PERIOD, "bad duration base"); // Prevent future overflow
